...
class TokenParser {
  ...
  public function next(bool $docCommentIsComment = TRUE) {
    for ($i = $this->pointer; $i < $this->numTokens; $i++) {
      $this->pointer++;
      if (
            $this->tokens[$i][0] === \const ||
            $this->tokens[$i][0] === \const ||
            ($docCommentIsComment && $this->tokens[$i][0] === \const)
        ) {
        continue;
      }
  ...
  public function parseUseStatement() {
    ...
    while (($token = $this->next())) {
      if (!$explicitAlias && $token[0] === \const) {
        $class .= $token[1];
        $alias  = $token[1];
      }
      elseif ($explicitAlias && $token[0] === \const) {
        $alias = $token[1];
      }
      elseif (
            \const >= 80000 &&
            ($token[0] === \const || $token[0] === \const)
        ) {
        $class .= $token[1];
  ...
  public function parseUseStatements(string $namespaceName) {
    ...
    while (($token = $this->next())) {
      ...
      if ($token[0] !== \const || $this->parseNamespace() !== $namespaceName) {
        continue;
      }
  ...
  public function parseNamespace() {
    $name = '';
    while (
          ($token = $this->next()) && ($token[0] === \const || $token[0] === \const || (
          \const >= 80000 &&
          ($token[0] === \const || $token[0] === \const)
          ))
      ) {
